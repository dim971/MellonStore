...
@objc public class MellonStore: NSObject {
    ...
    private lazy var managedObjectModel: NSManagedObjectModel? = {
        // Fetch Model URL
        print("modelName: \(String(describing: MellonStore.modelName))")
        
        let modelURL: URL
        if let url = Bundle.main.url(forResource: MellonStore.modelName!, withExtension: self.extensionName) {
            modelURL = url
        } else if let url = Bundle.init(for: MellonStore.self).url(forResource: MellonStore.modelName!, withExtension: self.extensionName) {
            modelURL = url
        } else {
            return nil
        }
    ...
    init(
        storeName: String,
        withExtension extensionName: String = "momd",
        inMemory: Bool = false,
        completion: CoreDataManagerCompletion? = nil
    ) throws {
    ...
    static public func setup(
        storeName: String = MellonStore.modelName,
        withExtension extensionName: String = "momd",
        inMemory: Bool = false,
        completion: CoreDataManagerCompletion? = nil
    ) {
    ...
    private func setupMellonStore(inMemory: Bool = false) {
    ...
    private func synchroneSetupMellonStore(inMemory: Bool = false) {
        _ = mainManagedObjectContext.persistentStoreCoordinator
        if inMemory {
            self.addInMemoryStore()
        } else {
    ...
    public func transaction(_ block: @escaping (NSManagedObjectContext) -> Void) {
    ...
    static public func switchTo(storeIdentifier: String, withExtension extensionName: String = "momd", completion: @escaping CoreDataManagerCompletion) {
    ...
    public func copyStore(for storeIdentifier: String) throws {
        let storeName = "\(storeIdentifier)"
        let fileManager = FileManager.default
        let documentsDirectoryURL = fileManager.urls(for: .documentDirectory, in: .userDomainMask)[0]
    ...
    static public func deleteStore(for storeIdentifier: String) throws {
        let storeName = "\(storeIdentifier)"
        let fileManager = FileManager.default
        let documentsDirectoryURL = fileManager.urls(for: .documentDirectory, in: .userDomainMask)[0]
